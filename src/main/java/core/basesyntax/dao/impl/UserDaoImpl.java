...
public class UserDaoImpl extends AbstractDao implements UserDao {
    ...
    public User get(Long id) {
        try (Session session = factory.openSession()) {
            return session.createQuery("FROM User u LEFT JOIN FETCH "
                            + "u.comments WHERE u.id = :id", User.class)
                    .setParameter("id", id)
                    .uniqueResult();
        } catch (Exception e) {
    ...
    public List<User> getAll() {
        try (Session session = factory.openSession()) {
            return session.createQuery("FROM User u LEFT JOIN FETCH u.comments", User.class).list();
        } catch (Exception e) {
