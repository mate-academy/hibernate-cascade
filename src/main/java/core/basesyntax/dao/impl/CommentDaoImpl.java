...
public class CommentDaoImpl extends AbstractDao implements CommentDao {
    ...
    public List<Comment> getAll() {
        try (Session session = factory.openSession()) {
            String query = "SELECT a FROM Comment a";
            return session.createQuery(query, Comment.class).getResultList();
        } catch (Exception e) {
            throw new RuntimeException("Couldn't get list of comments", e);
        }
    }
    ...
    public void remove(Comment comment) {
        ...
        try {
            ...
            throw new RuntimeException("Couldn't remove comment " + comment, e);
        } finally {
            if (session != null) {
                session.close();
            }
